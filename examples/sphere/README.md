# Sphere — hors-render-v2

**Toolkit 0.7.5 · stable hors-render-v2 prebuilt examples.**

| Cartridge | Samples | Colours |
| --- | ---: | --- |
| [sphere-hors-render-v2.crt](sphere-hors-render-v2.crt) | 192 | Monochrome |

```bash
x64sc +easyflashcrtwrite -cartcrt examples/sphere/sphere-hors-render-v2.crt
```

Press SPACE when an identification screen is shown. Companion labels and manifests identify the exact build.

```bash
python3 tools/build_hors_v2_examples.py --only sphere
```
