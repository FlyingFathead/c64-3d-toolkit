...
| --- | ---: | --- |
| [cube-hors-render-v2.crt](cube-hors-render-v2.crt) | 192 | Monochrome |

```bash
x64sc +easyflashcrtwrite -cartcrt examples/cube/cube-hors-render-v2.crt
```

Press SPACE when an identification screen is shown. Companion labels and manifests identify the exact build.

```bash
python3 tools/build_hors_v2_examples.py --only cube
```
