...
| --- | ---: | --- | ---: | ---: |
| TORUS | 32 | yunroll, cart scaffold | 14.10 | +0.00% |
| TORUS DENSE | 32 | yunroll, cart scaffold | 12.42 | +0.00% |
| CUBE | 36 | yunroll, cart scaffold | 30.27 | +0.00% |
| SPHERE | 24 | yunroll, cart scaffold | 15.50 | +0.00% |
| HORSE HEAD | 32 | V9 | 13.86 | +0.73% |
| SUNFLOWER TORUS | 28 | V7, V8, V9 | 12.15 | +0.00% |
| SUNFLOWER COLOR | 20 | V9 | 10.55 | +0.96% |
| SPACE HORSE SPIN | 24 | V9 | 10.75 | +0.94% |
| SPACE HORSE CRAWL | 32 | V9 | 23.51 | +2.63% |
| FALLING CUBES | 18 | yunroll, cart scaffold | 14.90 | +0.00% |
| HORSE HEAD HIFI | 128 | V9 | 10.25 | +0.99% |
| SUNFLOWER TORUS HIFI | 128 | V9 | 14.46 | +16.13% |
...
- Full bitmap and colour verification covered **17,914 completed pictures**. Raw traces, cartridge hashes, per-entry oracle hashes, unsupported-build reasons and individual results remain in the external workspace.
...
This shared-controller matrix is the canonical cross-method lookup. It records
5,271 V9 FPS-preference flips versus 5,181 with V8 (+1.7%). The original native
V9 cartridge test recorded 5,274; differing launch/timer phase moves a few frames
across the fixed observation boundary. These are different harnesses using the
same renderer bytes, not interchangeable samples. Compare within this matrix.

Pacing options use the existing nominal 50 Hz PAL convention. Five refreshes is
nominal 10 FPS; cycle-based measurements report about 10.025 because actual PAL
refresh is about 50.125 Hz. The reported values retain that precision. An exact
10.000 Hz cadence cannot simultaneously consist of equal whole PAL refresh holds.
No decimal FPS display was added to any production demo or cartridge.

## What the five resident wins suggest

| Demo | Resident yunroll average FPS | V9 average FPS | V9 fetch mean cycles (diagnostic) |
| --- | ---: | ---: | ---: |
| TORUS | 14.10 | 13.46 | 8,702 |
| TORUS DENSE | 12.42 | 12.15 | 9,827 |
| CUBE | 30.27 | 26.82 | 5,752 |
| SPHERE | 15.50 | 14.66 | 9,052 |
| FALLING CUBES | 14.90 | 13.86 | 10,308 |

These five datasets fit the resident table layout and use vector pictures in
V9. The resident path avoids streaming those records into staging RAM. V9's
direct-ROM byte-picture improvement therefore does not help these particular
pictures. Isolated V9 stage profiles confirm a measurable fetch cost, but do
not establish that fetching explains the entire difference: clearing, kernels
and buffer scheduling also differ. The FPS values above come from normal PLAY
ALL; stage timings are explanatory diagnostics, not a second FPS ranking.

RAM fit is a constraint, not an automatic recommendation. A future unified
selector must first reserve RAM and CPU time for music, interrupts and other
application features, then compare feasible backends. Streaming avoids keeping
the entire animation resident, but still needs the fixed graphics buffers,
staging and caches listed above. Neither streaming nor resident rendering alone
guarantees music compatibility: actual address ranges, bank switching and
interrupt latency must be checked with the chosen player.

The current matrix's per-demo best FPS-preference counts would sum to 5,457
versus V9's 5,271 (+3.5%). This is only a hypothetical selection total; no unified
runtime has been implemented or measured, and it excludes selection/loading
