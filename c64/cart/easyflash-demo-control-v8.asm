; V8 EasyFlash controls, with a PLAY ALL timer in menu-owned RAM at $c700.
;
...
cart_control_auto_next:
    jmp control_next_nowait
...
cart_control_irq_impl:
    ...
    lda #0
    sta CONTROL_LATCH
    jsr $c700                   ; menu-owned PLAY ALL timer
control_chain:
    pla
    tay
    pla
...
.if * > $02f7
    .error "EasyFlash demo control code overlaps control variables at $02F7 (V8 exhibition flag)"
.endif
.fill $0300-*, $ff
