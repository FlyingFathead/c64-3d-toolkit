...
CONTROL_RAM       = $0200
CONTROL_EXHIBITION = $02f7      ; persistent V8-only exhibition flag
...
; F5 is INTERNAL DEMO MODE for exhibitions, never an A/B benchmark.
; ONLY use normal PLAY ALL for A/B comparisons between methods and versions.
; F5 MUST NOT be used for benchmarking. Keep normal timing uniform.
menu_exhibition:
    jsr wait_menu_key_release
    lda #1
    sta CONTROL_EXHIBITION
...
install_control_shim:
    ; $02F7-$02FF is persistent control state, not part of the executable shim.
    ; Copy only $0200-$02F6 so reinstalling the shim while returning to the
...
; Return A=0 none, 1 next/down/right, 2 previous/up/left, 3 RETURN, 4 F1 style, 5 F5 internal demo.
...
title_default:
    .text "C64 3D TOOLKIT 0.6.8  ALL V"
    .byte $30+RENDERER_VERSION
    .byte 0
title_fancy:
    .text "C64-3D-TOOLKIT 0.6.8"
    .byte 0
subtitle_fancy:
...
.if RENDERER_VERSION >= 7
; A/B testing uses normal PLAY ALL (10 seconds per demo by default).
; Keep its cadence intact; only the explicit F5 flag selects longer HiFi holds.
setup_play_all:
    lda selected_entry
    cmp #$ff
...
.endif
    ...
    .text " SECONDS EACH)"
    .byte 0
.include "play-all-durations.inc"
.endif
