; yunroll-cart-v10-scene: paged directory, 16-bit frames, paced presentation.
; V9 drawing and direct metadata loading with paged, paced scene playback.
; yunroll-cart-v9: selective-clear EasyFlash vector renderer.
; V9 adds tagged byte-clear spans. Inherits V6 X kernels/direct metadata and
; V5 lossless picture reuse; PREFER_RAM selects smaller Y kernels.
; Copies line records into $a000-$bfff and metadata directly into the recycled
; slot cache, in interruptible bursts of at most 256 bytes.
; 16-bit visible-run count. Main rasteriser and triple-buffer handoff retained.
; All rendering happens with cartridge off and $01=$35 (RAM under KERNAL).
; Boot does not call KERNAL; initialise VIC registers explicitly.
; -----------------------------------------------------------------------------
; C64-3D-TOOLKIT GENERATED SPINNER
...
; V3 also owns $e0-$ef for hot counters; $f0-$f7 are the shared pointers.
; All scratch is assigned before use. Raster IRQs preserve it.
...
start:
        ...
        lda #$00
        sta $d015                   ; disable uninitialised sprites
        lda #$03
        sta $dd02                   ; VIC bank select pins are outputs
        lda #0
        sta $d020
        sta $d021
        ...
        lda #$3b                    ; display on, bitmap, 25 rows, Y scroll 3
        sta $d011

        lda #$08                    ; 40 columns, hires, X scroll 0
        sta $d016
...
frame_draw_complete:
        ...
        ldx render_slot
        lda #0 ; slot metadata valid, independent of frame number
        sta slot_frame,x
...
; High bit in the span offset selects a byte count. The original metadata
; footprint and colour-span offsets are unchanged. Y counts backwards so the
; byte loop needs no separate X decrement or per-byte index comparison.
cofs_byte_span:
        and #$1f
        clc
        adc draw_base_hi
...
apply_current_frame_colors:
        ...
        ; Skip count + 3*count bytes using a 16-bit multiply/add.
        ldy #0
        lda (STREAM_LO),y
        sta color_skip_temp
        lda #0
        ...
        clc
        adc color_skip_temp
        bcc acfc_skip_mul_ok
        inc color_skip_hi
acfc_skip_mul_ok:
        clc
        adc #1
        bcc acfc_skip_one_ok
        inc color_skip_hi
acfc_skip_one_ok:
        clc
        adc STREAM_LO
...
.endif
        ...
        ora lines_remaining
        beq dcl_done
        ; Low byte counts the initial partial batch, then wraps for 256.
        ; High byte counts batches; zero represents 256 only at the limit.
        lda lines_remaining
        beq dcl_batches_ready
...
; -----------------------------------------------------------------------------
; V6 partial X chunks accumulate adjacent bits in A. Each bitmap byte
; is written only when Y changes, the X cell ends, or the line finishes.
...
; -----------------------------------------------------------------------------
.if PREFER_RAM
; Compact Y-major loops: share all eight entry phases. X kernels stay unchanged.
ymp0:
ymp1:
ymp2:
ymp3:
...
color_skip_temp = $e8
color_skip_hi = $e9
color_spans_remaining = $ea
color_screen_base_hi = $eb
color_cells_temp = $ec
color_value_temp = $ed
...
; V5 full Y blocks: eight pixels, one count update.
.if PREFER_RAM == 0
...
reset_old_frame_colors:
        ...
        clc
        adc color_skip_temp
        bcc rofc_skip_mul_ok
        inc color_skip_hi
rofc_skip_mul_ok:
        clc
        adc #1
        bcc rofc_skip_one_ok
        inc color_skip_hi
rofc_skip_one_ok:
        clc
        adc STREAM_LO
...
.endif
        ...
        .include "generated/cart-v10-scene-helper.inc"
        .include "generated/tables.inc"


; V3 lookup pages do not grow the frame/cache allocations.
* = $4300
v3_entry_hi:
        .byte >xmp0,>xmp1,>xmp2,>xmp3,>xmp4,>xmp5,>xmp6,>xmp7,>xmp0,>xmp1,>xmp2,>xmp3,>xmp4,>xmp5,>xmp6,>xmp7
        .byte >xmp0,>xmp1,>xmp2,>xmp3,>xmp4,>xmp5,>xmp6,>xmp7,>xmp0,>xmp1,>xmp2,>xmp3,>xmp4,>xmp5,>xmp6,>xmp7
        .byte >xmp0,>xmp1,>xmp2,>xmp3,>xmp4,>xmp5,>xmp6,>xmp7,>xmp0,>xmp1,>xmp2,>xmp3,>xmp4,>xmp5,>xmp6,>xmp7
        .byte >xmp0,>xmp1,>xmp2,>xmp3,>xmp4,>xmp5,>xmp6,>xmp7,>xmp0,>xmp1,>xmp2,>xmp3,>xmp4,>xmp5,>xmp6,>xmp7
        .byte >ymp0,>ymp0,>ymp0,>ymp0,>ymp0,>ymp0,>ymp0,>ymp0,>ymp1,>ymp1,>ymp1,>ymp1,>ymp1,>ymp1,>ymp1,>ymp1
        .byte >ymp2,>ymp2,>ymp2,>ymp2,>ymp2,>ymp2,>ymp2,>ymp2,>ymp3,>ymp3,>ymp3,>ymp3,>ymp3,>ymp3,>ymp3,>ymp3
        .byte >ymp4,>ymp4,>ymp4,>ymp4,>ymp4,>ymp4,>ymp4,>ymp4,>ymp5,>ymp5,>ymp5,>ymp5,>ymp5,>ymp5,>ymp5,>ymp5
        .byte >ymp6,>ymp6,>ymp6,>ymp6,>ymp6,>ymp6,>ymp6,>ymp6,>ymp7,>ymp7,>ymp7,>ymp7,>ymp7,>ymp7,>ymp7,>ymp7
        .byte >xmn0,>xmn1,>xmn2,>xmn3,>xmn4,>xmn5,>xmn6,>xmn7,>xmn0,>xmn1,>xmn2,>xmn3,>xmn4,>xmn5,>xmn6,>xmn7
        .byte >xmn0,>xmn1,>xmn2,>xmn3,>xmn4,>xmn5,>xmn6,>xmn7,>xmn0,>xmn1,>xmn2,>xmn3,>xmn4,>xmn5,>xmn6,>xmn7
        .byte >xmn0,>xmn1,>xmn2,>xmn3,>xmn4,>xmn5,>xmn6,>xmn7,>xmn0,>xmn1,>xmn2,>xmn3,>xmn4,>xmn5,>xmn6,>xmn7
        .byte >xmn0,>xmn1,>xmn2,>xmn3,>xmn4,>xmn5,>xmn6,>xmn7,>xmn0,>xmn1,>xmn2,>xmn3,>xmn4,>xmn5,>xmn6,>xmn7
        .byte >ymn0,>ymn0,>ymn0,>ymn0,>ymn0,>ymn0,>ymn0,>ymn0,>ymn1,>ymn1,>ymn1,>ymn1,>ymn1,>ymn1,>ymn1,>ymn1
        .byte >ymn2,>ymn2,>ymn2,>ymn2,>ymn2,>ymn2,>ymn2,>ymn2,>ymn3,>ymn3,>ymn3,>ymn3,>ymn3,>ymn3,>ymn3,>ymn3
        .byte >ymn4,>ymn4,>ymn4,>ymn4,>ymn4,>ymn4,>ymn4,>ymn4,>ymn5,>ymn5,>ymn5,>ymn5,>ymn5,>ymn5,>ymn5,>ymn5
        .byte >ymn6,>ymn6,>ymn6,>ymn6,>ymn6,>ymn6,>ymn6,>ymn6,>ymn7,>ymn7,>ymn7,>ymn7,>ymn7,>ymn7,>ymn7,>ymn7
* = $4f00
v3_entry_lo:
        .byte <xmp0,<xmp1,<xmp2,<xmp3,<xmp4,<xmp5,<xmp6,<xmp7,<xmp0,<xmp1,<xmp2,<xmp3,<xmp4,<xmp5,<xmp6,<xmp7
        .byte <xmp0,<xmp1,<xmp2,<xmp3,<xmp4,<xmp5,<xmp6,<xmp7,<xmp0,<xmp1,<xmp2,<xmp3,<xmp4,<xmp5,<xmp6,<xmp7
...
yfull_end:
.if V5_REUSE_ENABLED
; Three exact ROM references identify the three resident pictures. Colour
; equality was checked by the host before assigning a shared ROM reference.
v5_find_picture:
        lda #0
        sta v5_reused
        ldx frame_index
...
v5_picture_found:
        ...
        cpy render_slot
        beq v5_find_done
; Let the earlier sample publish before reassigning a free/display slot.
v5_reuse_queue_wait:
        lda ready_slot
        cmp #$ff
        bne v5_reuse_queue_wait
        sei
        lda render_slot
        cpy display_slot
        beq v5_reuse_display
        sta free_slot
        sty render_slot
        cli
        rts
...
v5_hold_commit:
        ...
        rts
v5_reused: .byte 0
v5_saved_render: .byte 0
v5_slot_bank: .fill 3,0
v5_slot_lo: .fill 3,0
v5_slot_hi: .fill 3,0
.endif
v5_cold_end:
.if * > $9c00
.error "V5 Y kernel exceeds reserved RAM"
.endif
