#!/usr/bin/env python3
"""Build the three V6 examples from the released V4 vector samples.
...
For new Blender exports use c643d.py cart-stream --renderer yunroll-cart-v6-scene.
"""
...
def main():
    ...
    args=parser.parse_args(['cart-demos','--stream-renderer','yunroll-cart-v6',
    ...
    for hud in (True,False):
        stem='dont_lose_your_marbles-yunroll-cart-v6-scene'+('' if hud else '-clean')
        cartscene.assemble_scene(root,frames,scene,tass=tass,cartconv=conv,
            ...
            intro=True,ending=True,text_overlay=hud,renderer='yunroll-cart-v6-scene')
    record=dict(toolkit_version=__version__,renderer='yunroll-cart-v6',
    ...
    (root/'build/v6-example-provenance.json').write_text(json.dumps(record,indent=2)+'\n')
