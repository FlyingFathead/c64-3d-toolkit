#!/usr/bin/env python3
"""Cold-boot regression: original title, no timeout, explicit SPACE, optional help."""
import argparse
import hashlib
import json
...
def verify(crt, vice, data, output):
    ...
    rows = [('c64-3d-toolkit', 7), ('v. ' + meta['build_screen']['version'], 10),
            ('hors-renderer-v3', 12), ('github.com/FlyingFathead/c64-3d-toolkit', 16),
            ('SPACE to start', 20)]
    if interactive:
    ...
    result = dict(passed=True, cartridge=crt.name, sha256=hashlib.sha256(crt.read_bytes()).hexdigest(),
                  layout='v0.7.8, help on separate row 19 when interactive',
