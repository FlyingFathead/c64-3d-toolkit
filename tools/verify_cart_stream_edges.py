...
def check(tass,cartconv,vice,vice_data=None,renderer="yunroll-cart-v3"):
    ...
    for colors in (True,False):
        ...
        crt,m=assemble_cartridge(root,frames,mesh,tass=tass,cartconv=cartconv,outdir=out,stem=stem,colors=colors,renderer=renderer,optimize=False)
    ...
    crt,m=assemble_cartridge(root,fs,mesh,tass=tass,cartconv=cartconv,outdir=out,stem=variant+'-directory-255',colors=False,renderer=renderer,optimize=False)
...
if __name__=='__main__':
    ...
    p.add_argument('--renderer',choices=('yunroll-cart-v3','yunroll-cart-v4','yunroll-cart-v5','yunroll-cart-v6','yunroll-cart-v7'),default='yunroll-cart-v3')
