...
def prepare(root,renderer,tass,tass_args=(),sources=None,prefer="fps"):
    if renderer not in ('yunroll-cart-v2','yunroll-cart-v3','yunroll-cart-v4','yunroll-cart-v5','yunroll-cart-v6','yunroll-cart-v7', 'yunroll-cart-v8'):raise ValueError('unsupported uniform renderer')
    ...
    for index,demo in enumerate(sources):
        ...
        if load!=0x0801 or len(blob)-2>3*8192 or load+len(blob)-2>(0x6000 if variant in ("v5", "v6", "v7", "v8") else 0x5000):raise ValueError('uniform runtime exceeds three-bank RAM layout')
        ...
        if variant in ("v7", "v8"): info[-1]["preference"]=prefer
        if variant == "v8":
            info[-1]["wire_format"] = "v8-adaptive-vectors-byte-spans"
...
def play_all_durations(entries, seconds, renderer):
    ...
    return [15 if renderer == 'yunroll-cart-v8' and seconds == 10 and
...
def build(a, *, sources=None):
    ...
    if variant not in ('v7', 'v8') and seconds != 10: raise ValueError('--play-all-seconds requires V7 or V8')
    if prefer == 'ram' and variant not in ('v7', 'v8'): raise ValueError('--prefer ram requires V7 or V8')
    ...
    assemble_demo_control(tass=tass,tass_args=a.tass_args or (),source=cli.CART/('easyflash-demo-control-v8.asm' if variant == 'v8' else 'easyflash-demo-control-v7.asm' if variant == 'v7' else 'easyflash-demo-control.asm'),output=control,labels=work/f'{stem}-control.lbl',listing=work/f'{stem}-control.lst',cwd=root)
    ...
    if variant in ('v5', 'v6', 'v7', 'v8'): manifest['build_screen']=dict(version=__version__,renderer=f'yunroll-{variant}'+(' (ram)' if prefer == 'ram' else ''),ticks=150,skip_key='SPACE')
    if variant in ('v7', 'v8'):
