...
def pack_frames(frames,colors=True,*,chip="roml",first_bank=FIRST_DATA_BANK,aliases=None, encoder=frame_block):
...
def assemble_cartridge(root,frames,mesh,*,tass,cartconv,outdir,stem,tass_args=(),color_index=1,colors=True,renderer="yunroll-cart-v2",optimize=True,prefer="fps",hud=None):
    ...
    if load!=0x0801 or end>(0x6000 if variant in ("v5", "v6", "v7", "v8", "v9", "v10") else 0x5000):raise ValueError('runtime outside bootstrap RAM destination')
    ...
    (work/'oracle.json').write_text(json.dumps([asdict(f) for f in (original_frames if variant in ('v7', 'v8', 'v9', 'v10') else frames)]))
...
def prepare_menu_streams(root,*,tass,cartconv,tass_args=(),renderer="yunroll-cart-v2"):
    ...
    for name in ('horse_head_hifi','sunflower_torus_hifi'):
        ...
        emit_directory(gen/'tables.inc',directory,direct_bytes=variant in ('v9', 'v10'));emit_hud(gen/'hud.inc',label,len(mesh.vertices),len(mesh.edges))
