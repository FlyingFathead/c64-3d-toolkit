...
def verify(crt, vice, vice_data):
    ...
    with tempfile.TemporaryDirectory(prefix='c643d-play-all-') as temp:
        ...
        for index in range(count + 1):
            stop(menu['play_all_count'], menu['menu_launch'] if index == 0 else None)
            dump(f'{index}-start')
        ...
        cmd = [vice, '-console', '-pal', '+sound', '-warp', '-seed', '1',
        ...
        for index in range(count + 1):
            ...
            assert before[0x02fd:0x0300] == bytes((2, 50, seconds))
            ...
            samples.append(dict(entry=index % count, count_interval_cycles=elapsed))
    ...
    return dict(cartridge=crt.name, sha256=hashlib.sha256(crt.read_bytes()).hexdigest(),
                menu_style=meta['menu_style'], seconds_per_demo=seconds, ticks_per_demo=50*seconds, all_entries=True,
                ...
                method='PAL VICE; production handlers via monitor, no host key injection.')
...
if __name__ == '__main__':
    ...
    a = p.parse_args()
    result = verify(a.crt, a.vice, a.vice_data)
