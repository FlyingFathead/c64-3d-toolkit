#!/usr/bin/env python3
"""Build four V9 examples from the released V4 vector samples.
...
For new Blender exports use c643d.py cart-stream --renderer yunroll-cart-v9-scene.
"""
...
def main():
    ...
    args=parser.parse_args(['cart-demos','--stream-renderer','yunroll-cart-v9',
    ...
    for hud in (True,False):
        stem='dont_lose_your_marbles-yunroll-cart-v9-scene'+('' if hud else '-clean')+('-ram' if a.prefer == 'ram' else '')
        cartscene.assemble_scene(root,frames,scene,tass=tass,cartconv=conv,
            ...
            intro=True,ending=True,text_overlay=hud,renderer='yunroll-cart-v9-scene',prefer=a.prefer)
    ...
    cartscene.assemble_scene(root,frames,scene,tass=tass,cartconv=conv,
        ...
        stem='horse_and_sunflower-yunroll-cart-v9-scene'+('-ram' if a.prefer == 'ram' else ''),
        ...
        renderer='yunroll-cart-v9-scene',prefer=a.prefer)
    record=dict(toolkit_version=__version__,renderer='yunroll-cart-v9',preference=a.prefer,play_all_seconds=a.play_all_seconds,
    ...
    (root/('build/v9-example-provenance'+('-ram' if a.prefer == 'ram' else '')+'.json')).write_text(json.dumps(record,indent=2)+'\n')
