...
nav_count=13 if has_play_all else 12
...
for style in styles:
 ...
 for i in range(nav_count):
  expected_selection=([*range(1,12),255,0] if has_play_all else [*range(1,12),0])[i]
  mon += [f'g ${s["menu_down"]:04x}'];dump(style,f'down-{i}')
 for i in range(nav_count):
  expected_selection=([255,*range(11,-1,-1)] if has_play_all else [*range(11,-1,-1)])[i]
