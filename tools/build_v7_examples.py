#!/usr/bin/env python3
"""Build the three V7 examples from the released V4 vector samples.
...
For new Blender exports use c643d.py cart-stream --renderer yunroll-cart-v7-scene.
"""
...
def main():
    ...
    for hud in (True,False):
        stem='dont_lose_your_marbles-yunroll-cart-v7-scene'+('' if hud else '-clean')+('-ram' if a.prefer == 'ram' else '')
        cartscene.assemble_scene(root,frames,scene,tass=tass,cartconv=conv,
            ...
            intro=True,ending=True,text_overlay=hud,renderer='yunroll-cart-v7-scene',prefer=a.prefer)
    record=dict(toolkit_version=__version__,renderer='yunroll-cart-v7',preference=a.prefer,play_all_seconds=a.play_all_seconds,
    ...
    (root/('build/v7-example-provenance'+('-ram' if a.prefer == 'ram' else '')+'.json')).write_text(json.dumps(record,indent=2)+'\n')
