#!/usr/bin/env python3
"""Build four V10 examples from the released V4 vector samples.
...
For new Blender exports use c643d.py cart-stream --renderer yunroll-cart-v10-scene.
"""
...
def main():
    ...
    args=parser.parse_args(['cart-demos','--stream-renderer','yunroll-cart-v10',
    ...
    for hud in (True,False):
        stem='dont_lose_your_marbles-yunroll-cart-v10-scene'+('' if hud else '-clean')+('-ram' if a.prefer == 'ram' else '')
    ...
    cartscene.assemble_scene(root,frames,scene,tass=tass,cartconv=conv,
        ...
        stem='horse_and_sunflower-yunroll-cart-v10-scene'+('-ram' if a.prefer == 'ram' else ''),
    ...
    record=dict(toolkit_version=__version__,renderer='yunroll-cart-v10',preference=a.prefer,play_all_seconds=a.play_all_seconds,
