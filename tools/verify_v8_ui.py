#!/usr/bin/env python3
"""Verify V8 build/closing screens, Marbles endings and sunflower stage costs."""
import argparse
...
def main():
    ...
    for pref in ('','-ram'):
        ...
        for hud in ('','-clean'):
            crt=root/f'examples/cart_marbles/dont_lose_your_marbles-yunroll-cart-v8-scene{hud}{pref}.crt'
            tasks.append(('screen-marbles'+hud+pref,screen,crt))
            if hud:
                tasks.append(('ending'+pref,ending,crt))
    def run(task):
        name,fn,crt=task
        result=fn(crt,a.vice,a.vice_data,root/'build/v8-ui'/name)
