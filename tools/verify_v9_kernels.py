#!/usr/bin/env python3
"""Exercise V9 byte-span boundaries, mixed formats and resident reuse in VICE."""
import argparse
import json
...
def main():
    ...
    p.add_argument('--report',type=Path,default=Path('docs/benchmarks/v9/kernels.json'))
    a=p.parse_args()
    frames=[]
    ...
    for prefer in ('fps','ram'):
        crt,manifest=assemble_scene(root,frames,scene(frames,'V9 BYTE CHECK'),
            tass=a.tass,cartconv=a.cartconv,outdir=root/'build/v9-kernel-checks',
            stem='v9-byte-check-'+prefer,hud_text='V9 BYTE CHECK',frame_ticks=1,
            intro=False,ending=False,text_overlay=False,renderer='yunroll-cart-v9-scene',prefer=prefer)
        ...
        # Fill ROML through bank 63, continue in ROMH, then cross a directory
        # page. No deduplication: every identical picture owns a physical block.
        bank_frames=[sparse(range(1024))]*257
        bank_crt,bank_meta=assemble_scene(root,bank_frames,scene(bank_frames,'V9 BANK CHECK'),
            tass=a.tass,cartconv=a.cartconv,outdir=root/'build/v9-kernel-checks',
            stem='v9-bank-check-'+prefer,hud_text='V9 BANK CHECK',frame_ticks=1,
            intro=False,ending=False,text_overlay=False,renderer='yunroll-cart-v9-scene',prefer=prefer,optimize=False)
        assert {d['chip'] for d in bank_meta['frame_data']}=={'roml','romh'}
        report[prefer]['roml_romh_directory_boundary']=verify(bank_crt,a.vice,a.vice_data)
